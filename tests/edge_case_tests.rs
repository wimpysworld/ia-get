...
use ia_get::{
    cli::{Cli, Commands},
    metadata_storage::{ArchiveMetadata, ArchiveFile, DownloadSession, DownloadConfig},
    url_processing::{validate_and_process_url, extract_identifier_from_url},
    metadata::{get_json_url, parse_archive_metadata},
    filters::{parse_size_string, filter_files, format_size},
    error::IaGetError,
};
use clap::Parser;
...
fn test_cli_parsing_edge_cases() {
    ...
    let cli = Cli::parse_from([
        "ia-get",
        "--include-ext", "",
        "--exclude-ext", "a,b,c,d,e,f,g,h,i,j,k,l,m,n,o,p",
        "test"
    ]);
...
fn test_concurrent_edge_cases() {
    ...
    let filter_handles: Vec<_> = (0..5).map(|i| {
...
fn test_large_dataset_handling() {
    ...
    let filtered = filter_files(&large_metadata.files, &["txt".to_string()], &[], Some(500000));
