...
use clap::Parser;
use ia_get::{
    cli::Cli, error::IaGetError, metadata::get_json_url, url_processing::validate_and_process_url,
};
...
fn test_complete_user_workflow_progression() {
    ...
    assert_eq!(
        cli.url,
        Some("https://archive.org/details/example-archive".to_string())
    );
    ...
    assert!(
        ...
        "CLI validation should pass for valid input"
    );
    ...
    assert!(
        url_validation.is_ok(),
        "Valid archive.org URL should pass validation"
    );
    ...
    assert!(
        ...
        "Should generate metadata URL"
    );
    assert!(
        metadata_url.contains("example-archive"),
    ...
    // All preparatory steps completed successfully
...
fn test_error_handling_workflow() {
    ...
    let cli = Cli::parse_from(["ia-get", "https://example.com/not-archive"]);
    ...
    assert!(
        ...
        "Invalid URL should be caught in validation"
    );
...
fn test_user_interaction_patterns() {
    ...
    let minimal_cli = Cli::parse_from(["ia-get", "https://archive.org/details/test-archive"]);
    ...
    assert!(
        validation.is_ok(),
        "Minimal input should work with defaults"
    );
...
fn test_automatic_decision_making() {
    ...
    for (input, expected_type) in test_cases {
        ...
        assert!(
            metadata_url.contains("metadata"),
            "Should auto-generate metadata URL for {}",
            expected_type
        );
        assert!(
            metadata_url.contains("test"),
            "Should preserve identifier for {}",
            expected_type
        );
    ...
    assert_eq!(
        direct_cli.get_url(),
        Some("https://archive.org/details/direct-test")
    );
...
fn test_edge_case_workflow_handling() {
    ...
    assert!(
        validation_result.is_err(),
        "Empty URL should be rejected gracefully"
    );
...
fn test_recovery_mechanisms() {
    ...
    if let Err(error_msg) = validation_result {
        assert!(
            error_msg.contains("between 1 and 10"),
    ...
    let mixed_cli = Cli::parse_from([
        "ia-get",
        "--include-ext",
        "", // Empty extension list (unusual but not invalid)
        "--exclude-ext",
        "a,b,c,d,e,f,g,h,i,j", // Very long list (unusual but valid)
...
fn test_comprehensive_workflow_integration() {
    ...
    assert_eq!(
        metadata_url,
        "https://archive.org/metadata/comprehensive-test"
    );
