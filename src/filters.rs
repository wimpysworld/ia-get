...
pub fn parse_size_string(size_str: &str) -> Result<u64> {
    ...
    let (number_str, unit) = if size_str.ends_with("GB") || size_str.ends_with("TB") || 
                                size_str.ends_with("MB") || size_str.ends_with("KB") {
        (&size_str[..size_str.len()-2], &size_str[size_str.len()-2..])
...
pub fn filter_files<T: FilterOptions, F: FileEntry>(
    files: Vec<F>,
    options: &T,
) -> Vec<F> {
    let include_extensions: Option<Vec<String>> = options.include_ext().as_ref().map(|s| {
        s.split(',').map(|ext| ext.trim().to_lowercase()).collect()
    });
    
    let exclude_extensions: Option<Vec<String>> = options.exclude_ext().as_ref().map(|s| {
        s.split(',').map(|ext| ext.trim().to_lowercase()).collect()
    });
    
    let max_size = options.max_file_size().as_ref().and_then(|s| parse_size_string(s).ok());
...
mod tests {
    ...
    fn test_parse_size_string() {
        ...
        assert_eq!(parse_size_string("1.5MB").unwrap(), (1.5 * 1024.0 * 1024.0) as u64);
