...
impl InteractiveMenu {
    ...
    async fn configure_downloads(&mut self) -> Result<()> {
        ...
        loop {
            ...
            println!("  Output Path: {}", self.config.default_output_path.as_deref().unwrap_or("(current directory)").cyan());
            println!("  Concurrent Downloads: {}", self.config.concurrent_downloads.to_string().cyan());
            println!("  Max Retries: {}", self.config.max_retries.to_string().cyan());
            println!("  Resume Downloads: {}", if self.config.default_resume { "Yes".green() } else { "No".red() });
            println!("  Verbose Output: {}", if self.config.default_verbose { "Yes".green() } else { "No".red() });
            println!("  Log Hash Errors: {}", if self.config.default_log_hash_errors { "Yes".green() } else { "No".red() });
            println!();
    ...
    async fn configure_filters(&mut self) -> Result<()> {
        ...
        loop {
            ...
            println!("  Include Extensions: {}", self.config.default_include_ext.as_deref().unwrap_or("(none)").cyan());
            println!("  Exclude Extensions: {}", self.config.default_exclude_ext.as_deref().unwrap_or("(none)").cyan());
            println!("  Max File Size: {}", self.config.default_max_file_size.as_deref().unwrap_or("(unlimited)").cyan());
            println!();
    ...
    async fn view_recent_urls(&mut self) -> Result<()> {
        ...
        if self.config.recent_urls.is_empty() {
            ...
            println!("URLs will appear here after you use ia-get to download from Internet Archive.");
        } else {
            ...
            println!("2. Set Max Recent URLs (current: {})", self.config.max_recent_urls);
    ...
    async fn advanced_settings(&mut self) -> Result<()> {
        ...
        loop {
            ...
            println!("  HTTP Timeout: {} seconds", self.config.http_timeout.to_string().cyan());
            println!("  User Agent: {}", self.config.user_agent_override.as_deref().unwrap_or("(default)").cyan());
            println!("  Dry Run Mode: {}", if self.config.default_dry_run { "Yes".green() } else { "No".red() });
            println!();
    ...
    async fn preview_config(&mut self) -> Result<()> {
        ...
        println!("  Output Path: {}", self.config.default_output_path.as_deref().unwrap_or("(current directory)"));
        println!("  Concurrent Downloads: {}", self.config.concurrent_downloads);
        ...
        println!("  Include Extensions: {}", self.config.default_include_ext.as_deref().unwrap_or("(none)"));
        println!("  Exclude Extensions: {}", self.config.default_exclude_ext.as_deref().unwrap_or("(none)"));
        println!("  Max File Size: {}", self.config.default_max_file_size.as_deref().unwrap_or("(unlimited)"));
        println!();
        
        // Advanced settings
        println!("{}", "⚙️ Advanced Settings:".blue().bold());
        println!("  HTTP Timeout: {} seconds", self.config.http_timeout);
        println!("  User Agent: {}", self.config.user_agent_override.as_deref().unwrap_or("(default)"));
    ...
    async fn save_and_exit(&mut self) -> Result<()> {
        ...
        match ConfigManager::validate_config(&self.config) {
            Ok(()) => {
                ...
                println!("Config file: {}", self.config_manager.config_file_path().display().to_string().dimmed());
            }
            Err(e) => {
    ...
    fn set_include_extensions(&mut self) -> Result<()> {
        let exts = self.get_string_input("Enter file extensions to include (comma-separated, e.g., pdf,txt,mp3)")?;
        if exts.is_empty() {
    ...
    fn set_max_file_size(&mut self) -> Result<()> {
        loop {
            let size = self.get_string_input("Enter maximum file size (e.g., 100MB, 1GB, or press Enter to clear)")?;
            if size.is_empty() {
    ...
    fn create_preset(&mut self) -> Result<()> {
        ...
        let preset = FilterPreset {
            name,
            description,
            include_ext: if include_ext.is_empty() { None } else { Some(include_ext) },
            exclude_ext: if exclude_ext.is_empty() { None } else { Some(exclude_ext) },
            max_file_size: if max_file_size.is_empty() { None } else { Some(max_file_size) },
        };
    ...
    fn edit_preset(&mut self) -> Result<()> {
        ...
        loop {
            ...
            if choice >= 1 && choice <= self.config.filter_presets.len() {
                ...
                println!("{}", "Preset editing functionality would be implemented here.".yellow());
                break;
            } else {
                println!("{}", "Invalid preset number.".red());
            }
    ...
    fn view_preset_details(&mut self) -> Result<()> {
        ...
        loop {
            ...
            if choice >= 1 && choice <= self.config.filter_presets.len() {
                ...
                println!("Include Extensions: {}", preset.include_ext.as_deref().unwrap_or("(none)"));
                println!("Exclude Extensions: {}", preset.exclude_ext.as_deref().unwrap_or("(none)"));
                println!("Max File Size: {}", preset.max_file_size.as_deref().unwrap_or("(unlimited)"));
                break;
            } else {
                println!("{}", "Invalid preset number.".red());
            }
    ...
    fn reset_to_defaults(&mut self) -> Result<()> {
        println!("{}", "⚠️ This will reset ALL settings to defaults. Are you sure? (y/N)".yellow().bold());
    ...
    fn create_backup(&mut self) -> Result<()> {
        match self.config_manager.backup_config() {
            Ok(backup_path) => {
                println!("{} Backup created: {}", "💾".green(), backup_path.display().to_string().cyan());
            }
    ...
    fn restore_backup(&mut self) -> Result<()> {
        ...
        println!("{}", "Backup restoration functionality would be implemented here.".yellow());
        Ok(())
    }
    
    fn show_config_location(&mut self) -> Result<()> {
        println!("Configuration file location:");
        println!("  {}", self.config_manager.config_file_path().display().to_string().cyan());
        println!("Configuration directory:");
        println!("  {}", self.config_manager.config_directory().display().to_string().cyan());
        Ok(())
    }
}
