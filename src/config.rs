...
impl ConfigManager {
    ...
    pub fn get_config_directory() -> Result<PathBuf> {
        let config_dir = if cfg!(windows) {
            ...
            std::env::var("APPDATA")
                .map_err(|_| IaGetError::Config("APPDATA environment variable not found".to_string()))?
                .into()
        } else {
            // Unix-like: ~/.config/ia-get
            let home = std::env::var("HOME")
                .map_err(|_| IaGetError::Config("HOME environment variable not found".to_string()))?;
        ...
        if !ia_get_config_dir.exists() {
            fs::create_dir_all(&ia_get_config_dir)
    ...
    pub fn validate_config(config: &Config) -> Result<()> {
        if config.concurrent_downloads == 0 || config.concurrent_downloads > 20 {
            return Err(IaGetError::Config("Concurrent downloads must be between 1 and 20".to_string()));
        }
        
        if config.max_retries > 10 {
            return Err(IaGetError::Config("Max retries cannot exceed 10".to_string()));
        }
        
        if config.http_timeout == 0 || config.http_timeout > 300 {
            return Err(IaGetError::Config("HTTP timeout must be between 1 and 300 seconds".to_string()));
        }
        
        if config.max_recent_urls > 100 {
            return Err(IaGetError::Config("Max recent URLs cannot exceed 100".to_string()));
        }
        
        // Validate filter presets
        for preset in &config.filter_presets {
            if preset.name.is_empty() {
                return Err(IaGetError::Config("Filter preset names cannot be empty".to_string()));
            }
        }
    ...
    pub fn backup_config(&self) -> Result<PathBuf> {
        ...
        let backup_file = self.config_dir.join(format!("config_backup_{}.toml", timestamp));
    ...
    pub fn list_backups(&self) -> Result<Vec<PathBuf>> {
        ...
        for entry in entries {
            let entry = entry.map_err(|e| IaGetError::Config(format!("Failed to read directory entry: {}", e)))?;
            let path = entry.path();
...
pub mod endpoints {
    ...
    pub fn update_config(config: Config) -> ConfigResponse {
        match ConfigManager::validate_config(&config) {
            Ok(()) => {
                match ConfigManager::new().and_then(|manager| manager.save_config(&config)) {
                    Ok(()) => ConfigResponse {
                        success: true,
    ...
    pub fn get_filter_presets() -> Vec<FilterPreset> {
        get_config().config.map(|c| c.filter_presets).unwrap_or_default()
    }
    
    /// Get recent URLs
    pub fn get_recent_urls() -> Vec<String> {
        get_config().config.map(|c| c.recent_urls).unwrap_or_default()
    }
}
