...
async fn main() -> Result<()> {
    ...
    if dry_run {
        println!("{} DRY RUN MODE - fetching metadata only", "🔍".yellow().bold());
    }
    ...
    match fetch_and_display_metadata(
        ...
        Ok(_) => {
            if !dry_run {
                ...
                println!("📁 Output directory: {}", output_dir.display().to_string().bright_green());
            }
        }
        Err(e) => {
...
fn display_download_summary(session: &DownloadSession, output_dir: &Path) {
    ...
    println!("  💾 Total size: {}", format_size(total_bytes).bright_blue());
    ...
    if completed_files < total_files {
        ...
        println!("\n💡 Use {} to retry failed downloads", "--resume".bright_blue());
    }
}
...
async fn fetch_and_display_metadata(
    ...
    if dry_run {
        ...
        println!("   • Concurrent downloads: {}", concurrent_downloads.to_string().cyan());
        if !include_formats.is_empty() {
        ...
        match downloader.download_with_metadata(
            ...
            Ok(session) => {
                progress_bar.finish_with_message("✅ Download completed successfully!".green().bold().to_string());
                ...
                if !failed_files.is_empty() {
                    println!("\n{} {} files failed to download", "⚠️".yellow(), failed_files.len());
            ...
            Err(e) => {
                ...
                match &e {
                    ...
                    IaGetError::HashMismatch(msg) => {
                        ...
                        eprintln!("   {}", msg);
                        eprintln!("💡 This usually indicates network issues. Try downloading again.");
                    }
                    _ => {
                        eprintln!("\n{} Error: {}", "❌".red().bold(), e);
